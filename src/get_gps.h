...
{  
    int fd;  
         ...
         char* dev[] = {"/dev/ttyACM0","/dev/ttyACM1","/dev/ttyACM2"};  
    #else//使用USB转串口  
         char* dev[] = {"/dev/ttyACM0","/dev/ttyACM1","/dev/ttyACM2"};  
    #endif  
...
{  
    ...
    printf("The earth latitude is :%f\n",GPS_DATA.GPS_wd);  
    printf("The earth longitude is :%f\n",GPS_DATA.GPS_jd);   
...
void read_data(int fd,float* lat,float* lon)  
{  
            ...
            {  
                ...
                {                 
                        ...
                        {  
                            ...
                            SectionID=0;  
                        //printf("longitude data :%s\n",GPS_DATA.GPS_jd);
                        //print_info();  
                        char* lati;
                        char* longi;
                        *lat =strtof(GPS_DATA.GPS_wd,&lati)/100;
                        *lon =strtof(GPS_DATA.GPS_jd,&longi)/100;
                        //printf("&&&&&&&&&&&&&&%s\n",GPS_DATA.GPS_wd);
                        //printf("^^^^^^^^^^^^^^^%s\n",GPS_DATA.GPS_jd);
                        //if(strlen(GPS_DATA.GPS_jd)>0) break;
			sleep(1);
